...
**[快速开始](#快速开始)** · **[功能特性](#功能特性)** · **[部署说明](#部署说明)** · **[探索指南](#探索指南)** · **[配置说明](#配置说明)** · **[常见问题](#常见问题)** · **[构建说明](#构建说明)**
...
**Terrain Diffusion Next** 是 [terrain-diffusion-mc](https://github.com/xandergos/terrain-diffusion-mc) 的独立 fork（面向 Minecraft **26.x**）：将 [Terrain Diffusion](https://github.com/xandergos/terrain-diffusion)（SIGGRAPH '26）扩散模型地形生成器集成进 Minecraft，并在此基础上扩展了河流、海滩、含水层、矿脉与原版结构支持。客户端与专用服务器均可用，CUDA 构建自动下载运行库，开箱即用。

> [!NOTE]
> 本仓库为中文主文档；完整英文版见 [README_en.md](README_en.md)。构建相关细节见 [BUILDING.md](BUILDING.md)，CUDA 排查见 [CUDA_INSTALL.md](CUDA_INSTALL.md)。

**支持的版本**：本 Mod 面向 Minecraft **26.x**（26.1 / 26.2 / 26.3），单个 jar 声明兼容整个 `>=26.1` 范围。
...
2. 启动游戏（需联网一次）——AI 模型（约 2.5 GB）会自动下载并校验
3. 创建世界 → 世界类型选择「**地形扩散·Next**」
4. 可选：点击 **Customize** 设置 World Scale（见 [世界探索](#世界探索)）

> [!IMPORTANT]
> 需要 **Fabric + Fabric API**。CPU 构建很慢（macOS 除外），推荐使用 GPU 构建。
...
| Java | **25** |
| 显存 | 1.5 GB |
| 内存 | 2.5 GB（可能需要调高 Minecraft 内存分配） |
...
- **预加载区块**：`/td-preload <半径>` 在后台生成周围区块（配合关闭预加载使用）
- **探索世界**：`/td-explore` 在服务器本机启动地形浏览器（`http://127.0.0.1:19801`），进服前侦察地图

> [!WARNING]
> 若服务器 **没有 NVIDIA 驱动**（`libcuda.so.1` 缺失），CUDA 构建会自动回退 CPU（慢）。无 GPU 时请改用 **CPU 构建**（体积小得多，约 50 MB），详见 [CUDA_INSTALL.md](CUDA_INSTALL.md)。
...
> [!TIP]
> 想快速找奇特地形的坐标？先用 `/td-explore` 浏览整片大陆的高程与气候分布，记下高山、峡谷、群岛的坐标再传送过去。新区域按需生成，每个区域约 5 秒加载完成。
...
```properties
inference.device=gpu                       # 推理设备：cpu / gpu / auto
inference.offload_models=true              # 非活动模型移出显存，GPU 同一时刻只驻留一个模型
validate_model=true                        # 对已下载的模型文件做 SHA-256 校验
explorer.port=19801                        # 地形探索器网页端口（/td-explore 启动）
tile_size=256                              # 地形生成区域边长（方块），必须是 2 的幂
spawn_search.initial_size=16               # 出生点搜索初始区域（粗像素）
spawn_search.max_size=128                  # 出生点搜索最大区域（粗像素）
rivers.enabled=true                        # 是否启用河流系统
rivers.mode=hybrid                         # 河流模式：hybrid（沿地形）或 carver（纯噪声）
worldgen.skip_initial_chunk_preload=true   # 跳过启动预生成，加快就绪
...
<details>
<summary><b>DLL 初始化例程失败</b></summary>

需要 Java 25 或更高版本，已知最新版 [Microsoft OpenJDK 25](https://learn.microsoft.com/en-us/java/openjdk/download) 可用。
</details>

<details>
<summary><b>LoadLibrary failed with error 126</b>（仅 CUDA 构建）</summary>

CUDA 或 cuDNN 安装不当，排查步骤见 [CUDA_INSTALL.md](CUDA_INSTALL.md)。
</details>

<details>
<summary><b>Failed to load terrain-diffusion models</b></summary>

内存不足。模型约占 2.5 GB，请调高 Minecraft 内存分配。
</details>

<details>
<summary><b>服务器启动被 Watchdog 强制关闭</b></summary>

在 `server.properties` 中设置 `max-tick-time=-1` 禁用看门狗超时。
</details>

<details>
<summary><b>传送后服务器 tick 短暂卡顿</b></summary>

新区域地形生成需要数秒 GPU 推理，属正常现象，生成完成后恢复流畅。
</details>

<details>
<summary><b>如何快速找到奇特地形</b></summary>

运行 `/td-explore` 打开大地图查看各坐标的高程与气候，或分析粗地图数据自动找出最高峰与深谷的坐标。
</details>
